...
program.command('demo').description('Run the full sync storyline against an in-memory sheet (no token, nothing sent anywhere).').option('--fast', 'no pauses').action(async (o) => {
  const cfg = cfgOrDie();
  try { await runDemo(cfg, { fast: !!o.fast }); } catch (e) { die(e); }
});
