...
> on a sheet reliably says which item an old row belongs to, and guessing wrong would move your
> notes onto the wrong work.
...
| 18c | Review Owner | Text | who last set Human Review, "tool" or "human"; a checkbox says what the value is but not who chose it | sync engine | Yes | Overwritten |
| 18b | Repo Path | Text | the exact repository path this row is for; the machine-readable twin of Source, which is written for people | sync engine | Yes | Overwritten |
