...
> **Plan requirement.** *Manually observed on a live Smartsheet account on 2026-08-26; this is an
> external UI/account observation, not something this repository's tests can prove. Smartsheet may
> change plan gating at any time - re-check before relying on it.* API access needs a
...
1. `npx rsi export-csv` → writes `smartsheet_import.csv` and `column-definitions.json` into the configured `output.dir` (default `output`; the bundled sample overrides it to `output/orderly`). The command prints the exact path it wrote.
...
3. Smartsheet imports every column as Text/Number. Set the column types from `column-definitions.json`:
   ...
   - `Human Review`: **leave it as Text/Number on the CSV path.**
     > **Manually observed on a live sheet, 2026-08-26 (external observation, not proven by the test
> suite - the tests prove only what this exporter emits).** Converting the column to
