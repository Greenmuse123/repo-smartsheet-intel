...
"Deterministic parsing, evidence on every row, three field classes so human decisions survive, idempotent sync, dry-run, 137 tests, and a CSV path when there is no API. The optional Claude pass writes to AI Suggestion and may fill an empty Description with a labelled summary; it never writes a fact column."
